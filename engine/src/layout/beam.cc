...

{
  ...
  while(b) {
	...
	b->stem_h += c - a;
      ...
      else {  /* JSON_OUT — same geometry as PS */
	b->stem_top = b_base->stem_top + root_to_bottom;
	tt = b->get_v() - b_base->get_v();
	b->stem_h = tt + l_stem - root_to_bottom;
      }
  ...
  while (b) {
	...
	p->put_rule(inch_to_dvi(stem_w), b->stem_h);
      ...
      else {  /* JSON_OUT — negate stem_h: it is negative when beamup=1
	       * (l_stem = inch_to_dvi(-stem)), so pass -b->stem_h to get a
	       * positive rule height. Mirrors DVI_O logic but keeps DVI units.
	       * movev still uses the raw (negative) value so the cursor moves up. */
	p->movev(b->stem_h);
	p->put_rule(inch_to_dvi(stem_w), -b->stem_h);
      }
    }
	...
	p->put_rule(stem_w, dvi_to_inch(b->stem_h));
      ...
      else {  /* JSON_OUT — same as PS: DVI units */
	p->movev(b->stem_h);
	p->put_rule(stem_w, dvi_to_inch(b->stem_h));
      }
    }
