...
file_in::file_in(const char * fname, const char *mode)
{
    ...
    fn = (char *)malloc(1024);
    if (fn == NULL) {
        dbg0(Error, "file_in: malloc failed for filename\n");
        return;
    }
    /* Security: prevent buffer overflow with bounded copy */
    size_t fname_len = strlen(fname);
    if (fname_len >= 1024) {
        dbg0(Error, "file_in: filename too long (max 1023 chars)\n");
        free(fn);
        fn = NULL;
        return;
    }
...
file_in::file_in()
{
    ...
    /* Security: limit scanf to 1023 chars to prevent buffer overflow */
    scanf ("%1023s", fname);
