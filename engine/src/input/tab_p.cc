...
/* Moved from function-local statics inside format_page() so that
 * reset_tab_p_statics() can clear them between worker requests. */
static int fp_staff_page = 0;	/* number of systems per page */
static int fp_sys_count  = 0;	/* systems done on a page so far */

/* i_space: initialised to 0 at program start.  Defined here (formerly at the
 * do_system site below) so reset_tab_p_statics() can access it by name.
 * The original initialiser (= 0) is redundant for a static-storage variable
 * but kept semantically by the reset function. */
static int i_space = 0;

/* Called by reset_engine_state() in worker.cc before each layout command. */
void reset_tab_p_statics(void)
{
    fp_staff_page = 0;
    fp_sys_count  = 0;
    num_staff     = 0;
    /* Reset i_space to 0 so do_system recomputes it from interspace on the
     * first call of the new request. */
    i_space = 0;
}
		...
		font_list *f_a[], struct file_info *f)
{
    ...
    char cmdbuf[120];
    double staff_space;
    /* Use file-scope statics so reset_tab_p_statics() can clear them between
     * worker requests.  References behave identically to local statics. */
    int &staff_page = fp_staff_page;
    int &sys_count  = fp_sys_count;
...
/* i_space was originally defined here but moved to file-top so
 * reset_tab_p_statics() can access it between worker requests. */
