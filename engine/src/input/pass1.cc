...
/* Moved to file scope so reset_pass1_statics() can clear it between worker
 * requests.  Controls one-time Baroque chart-width setup inside pass1(). */
static int b_init_global=0;

/* S1 fix: snapshot of chart[] and big_chart[] as they exist at program start
 * (i.e. before any baroque request has run).  Baroque mode mallocs new width
 * strings into chart[2..6] and big_chart[2..6].  On reset we free those
 * malloc'd strings (anything whose pointer differs from the initial snapshot)
 * and restore the original pointer values so the next request starts clean.
 *
 * Initialised once in init_chart_snapshots() which is called from
 * reset_pass1_statics() on the very first call (snapshot_taken == 0). */
static int  snapshot_taken = 0;
static struct w chart_initial[10];
static struct w big_chart_initial[10];

static void init_chart_snapshots(void)
{
    int i;
    for (i = 0; i < 10; i++) {
        chart_initial[i]     = chart[i];
        big_chart_initial[i] = big_chart[i];
    }
    snapshot_taken = 1;
}

/* Called by reset_engine_state() in worker.cc before each layout command. */
void reset_pass1_statics(void)
{
    int i;
    if (!snapshot_taken) {
        /* First call: just take the snapshot; nothing to restore yet. */
        init_chart_snapshots();
    } else {
        /* Subsequent calls: free any width strings that baroque malloc'd,
         * then restore the original pointer values. */
        for (i = 0; i < 10; i++) {
            if (chart[i].width != chart_initial[i].width) {
                free(chart[i].width);
                chart[i].width = chart_initial[i].width;
            }
            if (big_chart[i].width != big_chart_initial[i].width) {
                free(big_chart[i].width);
                big_chart[i].width = big_chart_initial[i].width;
            }
        }
    }
    first_1       = 0;
    b_init_global = 0;
    /* old_width, old_weight, old_flag, l are static locals inside pass1().
     * l is re-assigned from listh (which is reset to NULL) at the top of
     * each pass1() call, so no action needed for l.
     * old_width/old_flag are only read after being assigned — safe. */
}
...
{
  int i, j;
  /* Use the file-scope b_init_global so reset_pass1_statics() can clear it
   * between worker requests.  Behaviour is identical to the original
   * function-scope static. */
  int &b_init = b_init_global;
  char c;
...
//      int ii;
	...
	for (i = 2; i < 8 /* STAFF*/; i++ ) {  // wbc jan 2025 was for i=2  STAFF = 11;
	  ...
	  if (d[i] != ' ' && d[i] != 'Q') {
		...
		if ( f->char_flag & ROB_CHAR ) {  // wbc jan 2025
		  fprintf(stderr, "pass1.c: adding to Robinson padding 1 \n");
		  l->padding += 0.01;;
		}
		else {
		  fprintf(stderr, "pass1.c: adding to padding 2 \n");
		  l->padding += 0.019;
		}
		break;
	      }
 ...
 */
  ...
  // printf(" pass1.cc  488 here ch %c l->dat[1] %s nxt %c next->next %c\n", *ch, l->dat, *nxt, l->next->next->dat[0]);
	...
	*/
    ...
    case '*':
      if (d[1] == 32)
	fprintf(stderr, "pass1: you need a value afer a star\n");
  ...
  if (f->flags & NO_EXPAND) {
    fprintf(stderr, "line length %f %f\n",
