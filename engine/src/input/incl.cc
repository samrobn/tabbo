...
void include(i_buf *ib, struct file_info *f, char file [])
{
    ...
    /* Security: block include directive when --no-includes flag is set */
    if (f->m_flags & NO_INCLUDES) {
        dbg0(Warning, "tab: include directive disabled (--no-includes)\n");
        return;
    }

    newf = (struct file_info *)malloc( sizeof (struct file_info));
    if (newf == NULL) {
        dbg0(Error, "tab: include: malloc failed for file_info\n");
        return;
    }

    newf->file = (char *)malloc( BUFSIZ );
    if (newf->file == NULL) {
        dbg0(Error, "tab: include: malloc failed for file name\n");
        free(newf);
        return;
    }
