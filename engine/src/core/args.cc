...
void set_o(const char *value, struct file_info *f)  {
...
#ifndef NO_STDOUT
  if (f->m_flags & JSON_OUT) {
    /* -json was already seen before -o; use .json extension.
     * Only append if the user-supplied path doesn't already end in .json
     * (after the .tab/.ps/.mid strips above). */
    size_t olen = strlen(f->out_file);
    if (olen < 5 || strcmp(f->out_file + olen - 5, ".json") != 0) {
      strncat(f->out_file, ".json ", 5);
    } else {
      strncat(f->out_file, " ", 1);
    }
  }
  else if (f->m_flags & SOUND ) {
  ...
  /* Emit to stderr directly: this diagnostic must never appear on stdout,
   * even if -worker follows -o on the command line (g_worker_mode is not yet
   * set when set_o runs, so dbg1 would otherwise send it to stdout). */
  if ( ! (f->m_flags & QUIET)) {
    fprintf(stderr, "tab: sending output to %s\n", f->out_file);
  }
#endif /* MAC */
}
...
void set_no_includes(const char *value, struct file_info *f)
{
  f->m_flags |= NO_INCLUDES;
}
void set_worker(const char *value, struct file_info *f)
{
  /* -worker implies -json (worker only emits NDJSON), -q (no banner noise
   * on stdout), and --no-includes (untrusted content arrives over stdin). */
  f->m_flags |= WORKER_MODE;
  f->m_flags |= JSON_OUT;
  f->m_flags |= QUIET;
  f->m_flags |= NO_INCLUDES;
  /* Clear all other output-mode bits — same housekeeping as set_json. */
  f->flags   &= ~PS;
  f->flags   &= ~PDF;
  f->flags   &= ~DVI_O;
  f->m_flags &= ~(unsigned long long)GS;
  f->m_flags &= ~(unsigned long long)SOUND;
  f->m_flags &= ~(unsigned long long)NMIDI;
  /* Enable the global worker-mode flag so dbg(Error,...) longjmps instead
   * of calling exit().  Done here (at arg-parse time) so any early fatal
   * errors during init also use longjmp rather than killing the process. */
  extern int g_worker_mode;
  g_worker_mode = 1;
}

void set_json(const char *value, struct file_info *f)
{
  f->m_flags |= JSON_OUT;
  /* JSON is a strict final-mode selector: clear all other output-mode bits
   * so that nothing downstream tries to invoke gs, write MIDI, etc.
   * Mirrors the "inverted" form of set_pdf / set_ps.
   *
   * Cast to unsigned long long before negating: GS (0x80000000) and NMIDI
   * (0x200000) are unsigned int constants.  ~GS without cast produces a
   * 32-bit mask that zero-extends to 64 bits, clearing all bits above
   * bit 30 and wiping JSON_OUT (bit 33). */
  f->flags   &= ~PS;
  f->flags   &= ~PDF;
  f->flags   &= ~DVI_O;
  f->m_flags &= ~(unsigned long long)GS;
  ...
  /* If -o appeared before -json on the command line, out_file already has
   * a .ps / .dvi / .mid suffix appended by set_o.  Patch it to .json so
   * the stderr message is truthful and the constructor extension-strip loop
   * in json_print has less work to do. */
  if (f->out_file && f->out_file[0]) {
    /* Strip trailing space (set_o appends a space after the extension) */
    size_t len = strlen(f->out_file);
    while (len > 0 && f->out_file[len - 1] == ' ') f->out_file[--len] = '\0';
    /* Strip known auto-added extensions */
    char *dot;
    while ((dot = strrchr(f->out_file, '.')) != 0) {
      if (strcmp(dot, ".ps")   == 0 ||
          strcmp(dot, ".dvi")  == 0 ||
          strcmp(dot, ".mid")  == 0 ||
          strcmp(dot, ".json") == 0) {
    ...
    strncat(f->out_file, ".json ", BUFSIZ - strlen(f->out_file) - 1);
    if (!(f->m_flags & QUIET)) {
      /* Use fprintf(stderr) directly: same stdout-safety reason as set_o. */
      fprintf(stderr, "tab: sending output to %s\n", f->out_file);
    }
  }
}
...
void args(int argc, char ** argv, struct file_info *f)
{
  ...
  struct tuple arglist[] = {
			    ...
			    {(char*)"no-includes", (void*) set_no_includes},
			    {(char*)"json", (void*) set_json},
			    {(char*)"worker", (void*) set_worker},
			    {(char*)0, (void*)0}
  };
  ...
  if (strlen(f->file) == 0) {
    /* Worker mode reads content from stdin — no file argument required. */
    if (f->m_flags & WORKER_MODE)
      return;
    show_usage();
