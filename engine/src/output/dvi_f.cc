...
/* Moved from function-local statics inside dvi_format() so
 * reset_dvi_f_statics() can clear them between worker requests. */
static double dvi_f_last_move  = 0.0;
static int    dvi_f_double_grid = 0;

/* Called by reset_engine_state() in worker.cc before each layout command. */
void reset_dvi_f_statics(void)
{
    grid_flag          = 0;
    dvi_f_last_move    = 0.0;
    dvi_f_double_grid  = 0;
...
struct list *l)			/* data */
{
  ...
  /* Use file-scope vars so reset_dvi_f_statics() can clear between requests. */
  double &last_move   = dvi_f_last_move;
  ...
  struct list *pp;
  int &double_grid    = dvi_f_double_grid;
