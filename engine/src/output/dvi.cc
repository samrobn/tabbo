...
/* Multiplies by the global `red` (font-size reduction set by -R8/-R9/etc.).
 * Use for content-space values (primitive coords, margins, staff_len_dvi) that
 * stay self-consistent with the renderer's coordinate system. For canvas /
 * physical-paper dimensions, use inch_to_dvi_unscaled — the renderer's
 * DVI_PER_INCH constant is the unscaled value, so a red-scaled canvas allocation
 * shrinks the page for reduced fixtures. */
int inch_to_dvi(const double inch)
{
    return ((int)(red * (inch * 254000.0) * ( (double)DENOM / (double)NUM)));
}

/* Same conversion as inch_to_dvi but always at red=1.0.
 * Used for page canvas dimensions that must be physical-paper size
 * regardless of the font-size reduction flag (-R). */
int inch_to_dvi_unscaled(double inch)
{
    return ((int)((inch * 254000.0) * ((double)DENOM / (double)NUM)));
}
