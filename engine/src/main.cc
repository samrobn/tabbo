...
void tfm_stuff(i_buf *b, file_info *f)
{
	...
	/* JSON layout output mode.  Use same font setup as PS path so that
...
#endif /* _WIN32 */
{
    ...
    *argv++;
...
#ifdef __unix__
    /* SIGALRM is only for one-shot CLI timeout behaviour.  In worker mode,
     * bun manages request timeouts; registering SIGALRM here would kill the
     * worker process after 17000 seconds regardless of activity. */
    ...
    /* Worker mode: enter long-lived NDJSON command loop.
     * Each command does its own file_stuff + tfm_stuff cycle.
     * The loop exits on stdin EOF; we then clean up and return 0. */
    if (f.m_flags & WORKER_MODE) {
        run_worker_loop(&f);
        if (f.scribe)   free(f.scribe);
        free(f.file);
        free(f.out_file);
        return 0;
    }
